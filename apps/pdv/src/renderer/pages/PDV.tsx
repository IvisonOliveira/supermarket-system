...
export default function PDV() {
  ...
  const updateQty = (index: number, qty: number) => {
    if (qty <= 0) return;
    setItems(prev => prev.map((item, i) => i === index ? { ...item, quantity: qty, subtotal: qty * item.price } : item));
    setTimeout(() => searchInputRef.current?.focus(), 10);
  };
  ...
  useEffect(() => {
    ...
    const cleanup = window.electronAPI.barcode.onScanned(async ({ code }: { code: string; isScanner: boolean }) => {
      try {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: globalThis.KeyboardEvent) => {
      ...
      if (e.key === 'F2') { 
         e.preventDefault(); 
         if (items.length > 0) {
            const newQty = window.prompt('Nova quantidade:', items[selectedIndex]?.quantity.toString() || '1');
      ...
      if (e.key === 'F8') {
         e.preventDefault();
         if (items.length > 0 && window.confirm('Deseja realmente cancelar a venda atual?')) clearSale();
      }
      if (e.key === 'F4' || e.key === 'Delete') {
         const activeEl = document.activeElement;
         if (activeEl === searchInputRef.current && e.key === 'Delete' && searchQuery.length > 0) return;
         e.preventDefault();
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden font-sans">
      ...
      <div className="w-[60%] flex flex-col border-r border-gray-700 bg-gray-800">
              ...
              <div className="flex-1 pr-4">
                ...
                <div className="flex items-center gap-6 text-gray-300">
                  <div className="flex items-center">
                    {item.isWeighing ? (
                      <>
                        <span className="w-24 text-center font-bold text-blue-400 text-3xl mr-4">{currentScaleWeight.toFixed(3)} kg</span>
                        <button onClick={() => {
                          const weight = currentScaleWeight > 0 ? currentScaleWeight : 1;
                          setItems(prev => prev.map((it, i) => i === index ? { ...it, quantity: weight, subtotal: weight * it.price, isWeighing: false } : it));
                        }} className="px-4 py-2 bg-blue-600 rounded hover:bg-blue-500 font-bold text-white transition-colors">Usar Peso</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => updateQty(index, item.quantity - 1)} className="px-3 py-1 bg-gray-600 rounded hover:bg-gray-500 font-bold text-xl transition-colors">-</button>
                ...
                <span className="w-36 text-right font-black text-white text-3xl">R$ {item.subtotal.toFixed(2)}</span>
                <button onClick={() => removeItem(index)} className="text-red-400 hover:text-red-300 ml-2 font-bold p-2 text-2xl transition-colors">✕</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-gray-900 p-8 border-t border-gray-700 shadow-[0_-10px_20px_rgba(0,0,0,0.1)]">
          <div className="flex justify-between items-end mb-6 text-gray-300">
             <div>
                <p className="text-2xl text-gray-400">Itens: <span className="font-bold text-white ml-2">{items.reduce((acc, i) => acc + i.quantity, 0)}</span></p>
                <div className="flex items-center mt-4 group">
                   <label htmlFor="discount" className="text-2xl text-gray-400">Desconto: <span className="text-white">R$</span></label>
                   <input 
                     id="discount"
                     type="number" 
             ...
             <div className="text-right">
                ...
                <p className="text-7xl font-black text-green-400 tracking-tighter">R$ {calculateTotal().toFixed(2)}</p>
             </div>
          </div>
          <div className="flex gap-4">
            <button onClick={clearSale} className="flex-1 bg-red-600 hover:bg-red-500 text-white text-2xl font-bold py-6 rounded-lg transition-colors shadow-lg active:scale-95">
              CANCELAR
            </button>
            <button 
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          {searchQuery.trim() ? (
            <div className="space-y-3">
              <h3 className="text-gray-500 mb-4 font-bold tracking-widest text-sm uppercase">Resultados ({searchResults.length})</h3>
              {searchResults.length === 0 ? (
                ...
                searchResults.map(p => (
                  ...
                  >
                    <div>
                      <div className="text-2xl font-bold text-gray-100 group-hover:text-white truncate max-w-[280px]">{p.name}</div>
                      <div className="text-base font-mono text-gray-400 mt-2">{p.barcode}</div>
                    </div>
                    <div className="text-2xl font-black text-green-400 group-hover:text-green-300">R$ {p.price.toFixed(2)}</div>
                  </button>
                ))
              )}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 2xl:grid-cols-3 gap-4">
                 {searchResults.slice(0, 12).map(p => (
                   ...
                   >
                     <span className="text-center w-full mb-3 text-xl font-semibold text-gray-300 group-hover:text-white line-clamp-3 leading-snug">{p.name}</span>
                     <span className="text-green-400 text-3xl font-black mt-auto">R$ {p.price.toFixed(2)}</span>
                   </button>
                 ))}
                 {searchResults.length === 0 && (
      ...
      <PaymentModal 
        ...
        onSuccess={() => { clearSale(); setIsPaymentOpen(false); }}
        items={items}
