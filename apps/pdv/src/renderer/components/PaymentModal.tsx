...
export default function PaymentModal({ isOpen, onClose, onSuccess, items, total, discount }: PaymentModalProps) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!isInputFocused) {
        if (e.key === '1') { e.preventDefault(); setActiveTab('dinheiro'); setTimeout(() => receivedInputRef.current?.focus(), 100); }
        if (e.key === '2') { e.preventDefault(); setActiveTab('pix'); }
        if (e.key === '3') { e.preventDefault(); setActiveTab('cartao'); }
      }
  ...
  const handleConfirm = async (method: PaymentMethod) => {
    try {
      ...
      alert('Venda registrada com sucesso!'); // Toast simplificado
      onSuccess();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-gray-900 border border-gray-700 w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden text-white font-sans">
        ...
        <div className="w-full md:w-1/3 bg-gray-800 p-8 border-r border-gray-700 flex flex-col">
          <h2 className="text-2xl text-gray-400 font-bold tracking-widest mb-2">TOTAL DA VENDA</h2>
          ...
          <div className="space-y-4 mt-auto">
            <button 
              onClick={() => { setActiveTab('dinheiro'); setTimeout(() => receivedInputRef.current?.focus(), 10) }}
          ...
          {activeTab === 'dinheiro' && (
            <div className="w-full max-w-md flex flex-col items-center space-y-8">
              <div className="w-full">
                <label className="text-2xl text-gray-400 font-bold block mb-4 text-center">Valor Recebido</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-4xl text-gray-500 font-black">R$</span>
                  <input 
              ...
              <div className="w-full bg-gray-800 p-6 rounded-xl border border-gray-700 text-center shadow-inner">
                ...
                <span className={`text-6xl font-black block ${change > 0 ? 'text-orange-400' : 'text-gray-500'}`}>
                  R$ {change.toFixed(2)}
                </span>
              </div>
          ...
          {activeTab === 'cartao' && (
            <div className="w-full max-w-md flex flex-col items-center space-y-10">
              <div className="w-full py-16 bg-gray-800 border border-gray-700 rounded-2xl flex flex-col items-center justify-center shadow-inner">
                <svg className="w-20 h-20 text-gray-600 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
